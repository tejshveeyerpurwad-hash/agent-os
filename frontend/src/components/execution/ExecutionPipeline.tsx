...
import { Bot, BrainCircuit, CheckCircle2, Loader2, Workflow, Terminal, ShieldCheck } from 'lucide-react'
...
export function ExecutionPipeline({ phase, completedCount = 0, totalCount = 0 }: ExecutionPipelineProps) {
  ...
  return (
    <div className="rounded-xl border border-dark-800 bg-dark-900/50 overflow-hidden" role="region" aria-label="Execution pipeline">
      <div className="px-3 sm:px-4 py-3 border-b border-dark-800 flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h2 className="text-sm font-semibold text-dark-200 truncate">Execution Pipeline</h2>
        </div>
        <span className={cn(
          'text-[11px] sm:text-xs font-medium px-2 py-0.5 rounded-full shrink-0',
      ...
      <div className="p-3 sm:p-4 lg:p-6">
        <div className="overflow-x-auto pb-2 -mx-1 px-1">
          <div className="flex items-start gap-0 min-w-[500px] sm:min-w-0">
            {stages.map((stage, i) => {
              ...
              return (
                <div key={stage.phase} className="flex-1 flex flex-col items-center text-center relative" aria-current={isCurrent ? 'step' : undefined}>
                  <div className={cn(
                    'w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center mb-2 transition-all duration-500 relative z-10',
                    isDone ? 'bg-emerald-500/10 text-emerald-400' :
                    isCurrent ? 'bg-primary-500/10 text-primary-400 ring-2 ring-primary-500/30' :
                    'bg-dark-800 text-dark-500',
                  )}>
                    {isDone ? <CheckCircle2 className="h-4 w-4 sm:h-5 sm:w-5" /> :
                     isCurrent && phase === 'executing' ? <Loader2 className="h-4 w-4 sm:h-5 sm:w-5 animate-spin" /> :
                     isCurrent && phase === 'awaiting_approval' ? <ShieldCheck className="h-4 w-4 sm:h-5 sm:w-5 text-amber-400" /> :
                     <Icon className="h-4 w-4 sm:h-5 sm:w-5" />}
                  </div>
                  <p className={cn(
                    'text-[10px] sm:text-[11px] font-medium leading-tight mb-0.5',
                  ...
                  {i < stages.length - 1 && (
                    <div className={cn(
                      'hidden lg:block absolute top-[18px] sm:top-5 left-[55%] w-[90%] h-px z-0',
        ...
        {totalCount > 0 && (
          <div className="mt-3 sm:mt-4 pt-3 border-t border-dark-800 flex items-center justify-between text-xs" role="group" aria-label="Progress">
            <span className="text-dark-500">
              Progress: {completedCount}/{totalCount} tasks
            </span>
            <div
              className="flex-1 mx-2 sm:mx-4 h-1.5 bg-dark-800 rounded-full overflow-hidden"
              role="progressbar"
              aria-valuenow={completedCount}
              aria-valuemin={0}
              aria-valuemax={totalCount}
              aria-label={`${Math.round((completedCount / totalCount) * 100)}% complete`}
            >
              <motion.div
                initial={{ width: 0 }}
            ...
            <span className="text-dark-500 font-medium shrink-0">
